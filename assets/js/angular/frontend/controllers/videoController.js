...
angular.module("kp_clothes").filter('commentfilter', function() {
...
}).controller('videoController',['$scope','$rootScope','$timeout','$state','$http','$stateParams','$log',function ($scope,$rootScope,$timeout,$state,$http,$stateParams,$log) {
   ...
   // $log.debug(Products.test);
    ...
    //Below code copied from videodetail controller
    $scope.videos = {};
    ...
    $scope.related_videos = {};
    $scope.get_related_videos = function(){
        $http.get($rootScope.base_url+'home/get_videos/')
            .success(function (data) {
                if(data!=null){
                    $scope.related_videos = data.videos;
                }
            })
            .error(function (data,status,code) {
                $log.debug(data);
    ...
    //above code copied from videodetail controller
